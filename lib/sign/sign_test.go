...
func TestSigningRSA(t *testing.T) {
	cert, err := tls.LoadX509KeyPair("../test/cert/sample_rsa.crt", "../test/cert/sample_rsa.pem")
	if err != nil {
...
func TestSigningECDSA(t *testing.T) {
	cert, err := tls.LoadX509KeyPair("../test/cert/sample_ecdsa.crt", "../test/cert/sample_ecdsa.pem")
	if err != nil {
		t.Error("Couldn't load sample certificate ", err)
		t.FailNow()
	}
